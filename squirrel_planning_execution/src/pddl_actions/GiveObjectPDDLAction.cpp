...
#include "GiveObjectPDDLAction.h"

namespace KCL_rosplan
{

GiveObjectPDDLAction::GiveObjectPDDLAction(ros::NodeHandle& node_handle)
{
	// knowledge interface
	...
	dispatch_sub_ = node_handle.subscribe("/kcl_rosplan/action_dispatch", 1000, &KCL_rosplan::GiveObjectPDDLAction::dispatchCallback, this);
}

GiveObjectPDDLAction::~GiveObjectPDDLAction()
{
	
}

void GiveObjectPDDLAction::dispatchCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg)
{
	...
	if (normalised_action_name != "give_object" || msg->parameters.size() != 4)
	{
		return;
	}
	
	ROS_INFO("KCL: (GiveObjectPDDLAction) Process the action: %s", normalised_action_name.c_str());
	...
	ROS_INFO("KCL: (GiveObjectPDDLAction) Process the action: %s, %s gives %s to %s at %s", normalised_action_name.c_str(), child.c_str(), object.c_str(), robot.c_str(), child_waypoint.c_str());
	...
	if (!update_knowledge_client_.call(knowledge_update_service)) {
		ROS_ERROR("KCL: (GiveObjectPDDLAction) Could not remove the previous (holding %s %s) predicate from the knowledge base.", robot.c_str(), object.c_str());
		exit(-1);
	}
	ROS_INFO("KCL: (GiveObjectPDDLAction) Removed the previous (holding %s %s) predicate from the knowledge base.", robot.c_str(), object.c_str());
	...
	knowledge_update_service.request.knowledge = kenny_knowledge;
	if (!update_knowledge_client_.call(knowledge_update_service)) {
		ROS_ERROR("KCL: (GiveObjectPDDLAction) Could not add the new (gripper_empty %s) predicate to the knowledge base.", robot.c_str());
		exit(-1);
	}
	ROS_INFO("KCL: (TakeObjectPDDLAction) Added the new (gripper_empty %s) predicate to the knowledge base.", robot.c_str());
	kenny_knowledge.values.clear();
	...
	knowledge_update_service.request.knowledge = kenny_knowledge;
	if (!update_knowledge_client_.call(knowledge_update_service)) {
		ROS_ERROR("KCL: (GiveObjectPDDLAction) Could not add the new (child_is_holding %s %s) predicate to the knowledge base.", child.c_str(), object.c_str());
		exit(-1);
	}
	ROS_INFO("KCL: (TakeObjectPDDLAction) Added the new (child_is_holding %s %s) predicate to the knowledge base.", child.c_str(), object.c_str());
	kenny_knowledge.values.clear();
