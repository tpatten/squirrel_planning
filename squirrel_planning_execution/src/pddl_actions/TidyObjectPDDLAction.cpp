...
#include "TidyObjectPDDLAction.h"
#include "squirrel_planning_execution/ViewConeGenerator.h"

namespace KCL_rosplan
{

TidyObjectPDDLAction::TidyObjectPDDLAction(ros::NodeHandle& node_handle)
{
	// knowledge interface
	...
	dispatch_sub_ = node_handle.subscribe("/kcl_rosplan/action_dispatch", 1000, &KCL_rosplan::TidyObjectPDDLAction::dispatchCallback, this);
}

TidyObjectPDDLAction::~TidyObjectPDDLAction()
{
	
}

void TidyObjectPDDLAction::dispatchCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg)
{
	...
	if (normalised_action_name != "tidy_object")// || msg->parameters.size() != 3)
	{
		return;
	}
	...
	kenny_knowledge.attribute_name = "tidy";
	...
	if (!update_knowledge_client_.call(knowledge_update_service)) {
		ROS_ERROR("KCL: (TidyObjectPDDLAction) Could not add the new (tidy %s) predicate to the knowledge base.", object.c_str());
		exit(-1);
	}
	ROS_INFO("KCL: (TidyObjectPDDLAction) Added the new (tidy %s) predicate to the knowledge base.", object.c_str());
