...
ClassifyObjectPDDLAction::ClassifyObjectPDDLAction(ros::NodeHandle& node_handle)
{
	// knowledge interface
...
void ClassifyObjectPDDLAction::dispatchCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg)
{
	...
	if (normalised_action_name != "classify_object" && 
	    ...
	    normalised_action_name != "finalise_classification_fail") // || msg->parameters.size() != 3)
	{
		return;
	}
