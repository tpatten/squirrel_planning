...
/* implementation of squirrel_planning_execution::RPSquirrelRoadmap visualisation */
namespace KCL_rosplan {

	/* output all waypoints */
	void RPSquirrelRoadmap::publishWaypointMarkerArray(ros::NodeHandle nh)
	{
	...
	void RPSquirrelRoadmap::clearMarkerArrays(ros::NodeHandle nh)
	{
		visualization_msgs::MarkerArray marker_array;
		size_t counter = 0;
		for (std::map<std::string, Waypoint*>::iterator wit=waypoints.begin(); wit!=waypoints.end(); ++wit) {
