...
#include "squirrel_planning_execution/pddl_generators/RobotKnowsSortingGamePDDLGenerator.h"

namespace KCL_rosplan {

void RobotKnowsSortingGamePDDLGenerator::generateProblemFile(const std::string& file_name, const KnowledgeBase& current_knowledge_base, const std::vector<const KnowledgeBase*>& knowledge_base, const Location& robot_location, const std::vector<const Location*>& locations, const std::vector<const Object*>& objects, const std::vector<const Box*>& boxes, const std::vector<const Type*>& types, const std::vector<const Child*>& children)
{
	...
	for (std::vector<const State*>::const_iterator ci = current_knowledge_base.states_.begin(); ci != current_knowledge_base.states_.end(); ++ci)
	{
		...
		for (std::vector<const Object*>::const_iterator ci = objects.begin(); ci != objects.end(); ++ci)
		{
			...
			if (object->location_ != NULL)
				myfile << "\t(object_at " << object->name_ << " " << object->location_->name_ << " " << state->state_name_ << ")" << std::endl;
			if (object->child_ != NULL)
				myfile << "\t(child_is_holding " << object->child_->name_ << " " << object->name_ << " " << state->state_name_ << ")" << std::endl;
		}
	...
	for (std::vector<const Child*>::const_iterator ci = children.begin(); ci != children.end(); ++ci)
	{
		const Child* child = *ci;
		myfile << "\t(child_at " << child->name_ << " " << child->location_->name_ << ")" << std::endl;
	}
...
void RobotKnowsSortingGamePDDLGenerator::generateDomainFile(const std::string& file_name, const KnowledgeBase& current_knowledge_base, const std::vector<const KnowledgeBase*>& knowledge_bases, const Location& robot_location, const std::vector<const Location*>& locations, const std::vector<const Object*>& objects, const std::vector<const Box*>& boxes, const std::vector<const Type*>& types, const std::vector<const Child*>& children)
{
	std::vector<const State*> states;
	...
	myfile << "\twaypoint robot object box type child" << std::endl;
	...
	myfile << "\t(child_is_holding ?c - child ?o - object ?s - state)" << std::endl;
	myfile << "\t(Rchild_is_holding ?c - child ?o - object ?s - state)" << std::endl;
	...
	myfile << "\t(examined ?o - object ?s - state)" << std::endl;
	myfile << "\t(Rexamined ?o - object ?s - state)" << std::endl;
	
	myfile << "\t(child_at ?c - child ?wp - waypoint)" << std::endl;
	...
	myfile << "\t; All the Locations." << std::endl;
	...
	myfile << "\t; The children." << std::endl;
	for (std::vector<const Child*>::const_iterator ci = children.begin(); ci != children.end(); ++ci)
	{
		myfile << "\t" << (*ci)->name_ << " - child" << std::endl;
	}
	...
	/**
	 * Examine an object, this is a contingent action, but we treat it as if the outcome is
	 * determined. Objects that could not be examined are given the type 'unknown'.
	 */
	myfile << "(:action observe-classifiable_on_attempt" << std::endl;
	myfile << "\t:parameters (?o - object ?v - robot ?wp ?wp2 - waypoint)" << std::endl;
	...
	for (std::vector<const State*>::const_iterator ci = states.begin(); ci != states.end(); ++ci)
	{
		...
		myfile << "\t\t\t(and" << std::endl;
		myfile << "\t\t\t\t(examined ?o " << (*ci)->state_name_ << ")" << std::endl;
		myfile << "\t\t\t\t(Rexamined ?o " << (*ci)->state_name_ << ")" << std::endl;
		myfile << "\t\t\t)" << std::endl;
		myfile << "\t\t)" << std::endl;
	}
	...
	for (std::vector<const State*>::const_iterator ci = states.begin(); ci != states.end(); ++ci)
	{
		...
		myfile << "\t\t\t\t(child_is_holding ?c ?o " << (*ci)->state_name_ << ")" << std::endl;
		myfile << "\t\t\t\t(Rchild_is_holding ?c ?o " << (*ci)->state_name_ << ")" << std::endl;
		myfile << "\t\t\t)" << std::endl;
		myfile << "\t\t)" << std::endl;
	}
	...
	myfile << "(:action take_object" << std::endl;
	myfile << "\t:parameters (?v - robot ?wp - waypoint ?o - object ?c - child)" << std::endl;
	myfile << "\t:precondition (and" << std::endl;
	myfile << "\t\t(not (resolve-axioms))" << std::endl;
	myfile << "\t\t(child_at ?c ?wp)" << std::endl;
	...
	for (std::vector<const State*>::const_iterator ci = states.begin(); ci != states.end(); ++ci)
	{
		myfile << "\t\t(when (m " << (*ci)->state_name_ << ")" << std::endl;
		myfile << "\t\t\t(and" << std::endl;
		myfile << "\t\t\t\t(not (child_is_holding ?c ?o " << (*ci)->state_name_ << "))" << std::endl;
		myfile << "\t\t\t\t(not (Rchild_is_holding ?c ?o " << (*ci)->state_name_ << "))" << std::endl;
	...
	*/
	...
	for (std::vector<const State*>::const_iterator ci = states.begin(); ci != states.end(); ++ci)
	{
		...
		for (std::vector<const Object*>::const_iterator ci = objects.begin(); ci != objects.end(); ++ci)
		{
			...
			myfile << "\t\t(when (or (examined " << object->name_ << " " << state->state_name_ << ") (not (m " << state->state_name_ << ")))" << std::endl;
			myfile << "\t\t\t(Rexamined " << object->name_ << " " << state->state_name_ << ")" << std::endl;
			myfile << "\t\t)" << std::endl;
			
			myfile << "\t\t(when (and (not (examined " << object->name_ << " " << state->state_name_ << ")) (m " << state->state_name_ << "))" << std::endl;
			myfile << "\t\t\t(not (Rexamined " << object->name_ << " " << state->state_name_ << "))" << std::endl;
			myfile << "\t\t)" << std::endl;
			/*
			...
			*/
			...
			{
				const Child* child = *ci;
				myfile << "\t\t(when (or (child_is_holding " << child->name_<< " " << object->name_ << " " << state->state_name_ << ") (not (m " << state->state_name_ << ")))" << std::endl;
				myfile << "\t\t\t(Rchild_is_holding " << child->name_<< " " << object->name_ << " " << state->state_name_ << ")" << std::endl;
				myfile << "\t\t)" << std::endl;
				
				myfile << "\t\t(when (and (not (child_is_holding " << child->name_<< " " << object->name_ << " " << state->state_name_ << ")) (m " << state->state_name_ << "))" << std::endl;
				myfile << "\t\t\t(not (Rchild_is_holding " << child->name_<< " " << object->name_ << " " << state->state_name_ << "))" << std::endl;
				myfile << "\t\t)" << std::endl;
			}
	...
	for (std::vector<const State*>::const_iterator ci = states.begin(); ci != states.end(); ++ci)
	{
		...
		for (std::vector<const State*>::const_iterator ci = states.begin(); ci != states.end(); ++ci)
		{
			...
			for (std::vector<const Object*>::const_iterator ci = objects.begin(); ci != objects.end(); ++ci)
			{
				...
				myfile << "\t\t(when (and (part-of " << state->state_name_ << " ?old_kb) (examined " << object->name_ << " " << state->state_name_ << ") (part-of " << state2->state_name_ << " ?new_kb))" << std::endl;
				myfile << "\t\t\t(and " << std::endl;
				myfile << "\t\t\t\t(not (Rexamined " << object->name_ << " " << state->state_name_ << "))" << std::endl;
				myfile << "\t\t\t\t(not (examined " << object->name_ << " " << state->state_name_ << "))" << std::endl;
				myfile << "\t\t\t\t(Rexamined " << object->name_ << " " << state2->state_name_ << ")" << std::endl;
				myfile << "\t\t\t\t(examined " << object->name_ << " " << state2->state_name_ << ")" << std::endl;
				myfile << "\t\t\t)" << std::endl;
				myfile << "\t\t)" << std::endl;
				/*
				...
				*/
				...
				{
					const Child* child = *ci;
					myfile << "\t\t(when (and (part-of " << state->state_name_ << " ?old_kb) (child_is_holding " << child->name_ << " " << object->name_ << " " << state->state_name_ << ") (part-of " << state2->state_name_ << " ?new_kb))" << std::endl;
					myfile << "\t\t\t(and " << std::endl;
					myfile << "\t\t\t\t(not (Rchild_is_holding " << child->name_ << " " << object->name_ << " " << state->state_name_ << "))" << std::endl;
					myfile << "\t\t\t\t(not (child_is_holding " << child->name_ << " " << object->name_ << " " << state->state_name_ << "))" << std::endl;
					myfile << "\t\t\t\t(Rchild_is_holding " << child->name_ << " " << object->name_ << " " << state2->state_name_ << ")" << std::endl;
					myfile << "\t\t\t\t(child_is_holding " << child->name_ << " " << object->name_ << " " << state2->state_name_ << ")" << std::endl;
					myfile << "\t\t\t)" << std::endl;
					myfile << "\t\t)" << std::endl;
				}
	...
	*/
	...
	for (std::vector<const Object*>::const_iterator ci = objects.begin(); ci != objects.end(); ++ci)
	{
		...
		for (std::vector<const State*>::const_iterator ci = states.begin(); ci != states.end(); ++ci)
		{
			...
			{
				const State* state2 = *ci;
				myfile << "\t\t\t\t(not (examined " << object->name_ << " " << state2->state_name_ << "))" << std::endl;
			}
			myfile << "\t\t\t\t(examined " << object->name_ << " " << state->state_name_ << ")" << std::endl;
			myfile << "\t\t\t)" << std::endl;
			myfile << "\t\t)" << std::endl;
		...
		for (std::vector<const Child*>::const_iterator ci = children.begin(); ci != children.end(); ++ci)
		{
			...
			for (std::vector<const State*>::const_iterator ci = states.begin(); ci != states.end(); ++ci)
			{
				...
				{
					const State* state2 = *ci;
					myfile << "\t\t\t\t(not (child_is_holding " << " " << child->name_ << " " << object->name_ << " " << state2->state_name_ << "))" << std::endl;
				}
				myfile << "\t\t\t\t(child_is_holding " << " " << child->name_ << " " << object->name_ << " " << state->state_name_ << ")" << std::endl;
				myfile << "\t\t\t)" << std::endl;
				myfile << "\t\t)" << std::endl;
			}
		}
...
void RobotKnowsSortingGamePDDLGenerator::createPDDL(const std::string& path, const std::string& domain_file, const std::string& problem_file, const std::string& robot_location_predicate, const std::map<std::string, std::string>& object_to_location_mapping, std::map<std::string, std::vector<std::string> >& near_waypoint_mappings, const std::map<std::string, std::string>& object_to_type_mapping, const std::map<std::string, std::string>& box_to_location_mapping, const std::map<std::string, std::string>& box_to_type_mapping, const std::vector<std::string>& children_predicates, const std::map<std::string, std::string>& object_to_child_mapping, const std::map<std::string, std::string>& child_location_mapping)
{
	// Now generate the waypoints / boxes / toys / etc.
	std::vector<const Location*> locations;
	std::vector<Location*> connected_locations;
	std::vector<const Box*> boxes;
	std::vector<const Object*> objects;
	std::vector<const Type*> types;
	std::vector<const Child*> children;
	
	// Create the children and the location of where they are located.
	for (std::map<std::string, std::string>::const_iterator ci = child_location_mapping.begin(); ci != child_location_mapping.end(); ++ci) {
		const std::string& child_predicate = (*ci).first;
		const std::string& location_predicate = (*ci).second;
		
		Location* location = new Location(location_predicate, false);
		Child* child = new Child(child_predicate, *location);
		...
		if (mi != near_waypoint_mappings.end())
		{
			...
			for (std::vector<std::string>::const_iterator ci = near_locations.begin(); ci != near_locations.end(); ++ci)
			{
				...
				location->connected_locations_.push_back(near_location);
				near_location->connected_locations_.push_back(location);
				connected_locations.push_back(near_location);
			}
		}
	}
	
	// Create the objects that are hold by children.
	for (std::map<std::string, std::string>::const_iterator ci = object_to_child_mapping.begin(); ci != object_to_child_mapping.end(); ++ci) {
		const std::string& object_predicate = (*ci).first;
		const std::string& child_predicate = (*ci).second;
		
		const Child* child = NULL;
		for (std::vector<const Child*>::const_iterator ci = children.begin(); ci != children.end(); ++ci)
		{
			const Child* c = *ci;
			if (c->name_ == child_predicate)
			{
				child = c;
				break;
			}
		...
		Object* object = new Object(object_predicate, *child);
	...
	for (std::map<std::string, std::string>::const_iterator ci = box_to_location_mapping.begin(); ci != box_to_location_mapping.end(); ++ci) {
		...
		if (mi != near_waypoint_mappings.end())
		{
			...
			for (std::vector<std::string>::const_iterator ci = near_locations.begin(); ci != near_locations.end(); ++ci)
			{
				...
				Location* near_location = new Location(near_location_name, false);
				locations.push_back(near_location);
				box_location->near_locations_.push_back(near_location);
				box_location->connected_locations_.push_back(near_location);
				near_location->connected_locations_.push_back(box_location);
				connected_locations.push_back(near_location);
			}
		}
	...
	connected_locations.push_back(robot_location);
	...
	for (std::vector<const Object*>::const_iterator ci = objects.begin(); ci != objects.end(); ++ci)
	{
		...
		for (std::vector<const Type*>::const_iterator ci = types.begin(); ci != types.end(); ++ci)
		{
			const Type* type = *ci;
			ss.str(std::string());
			ss << "kb_" << object->name_ << "_" << type->name_;
			type_mapping[object] = type;
			State* type_state = new State(ss.str(), type_mapping);
			kb_location->addState(*type_state);
			++state_id;
		}
	}
	
	// Connect the locations.
	for (std::vector<Location*>::const_iterator ci = connected_locations.begin(); ci != connected_locations.end(); ++ci)
	{
		Location* l = *ci;
		for (std::vector<Location*>::const_iterator ci2 = ci + 1; ci2 != connected_locations.end(); ++ci2)
		{
			Location* l2 = *ci2;
			l->connected_locations_.push_back(l2);
			l2->connected_locations_.push_back(l);
		}
	}
	
	for (std::vector<const Location*>::const_iterator ci = locations.begin(); ci != locations.end(); ++ci)
	{
		const Location* loc = *ci;
		std::cout << "Location: " << loc->name_ << std::endl;
		std::cout << "Connected to: " << std::endl;
		for (std::vector<const Location*>::const_iterator ci2 = loc->connected_locations_.begin(); ci2 != loc->connected_locations_.end(); ++ci2)
		{
			std::cout << "\t" << (*ci2)->name_ << "." << std::endl;
		}
	}
	
	ss.str(std::string());
	ss << path << domain_file;
	ROS_INFO("KCL: (RobotKnowsSortingGamePDDLGenerator) Generate domain... %s", ss.str().c_str());
	generateDomainFile(ss.str(), basis_kb, knowledge_bases, *robot_location, locations, objects, boxes, types, children);
	ss.str(std::string());
	ss << path  << problem_file;
	ROS_INFO("KCL: (RobotKnowsSortingGamePDDLGenerator) Generate problem... %s", ss.str().c_str());
	generateProblemFile(ss.str(), basis_kb, knowledge_bases, *robot_location, locations, objects, boxes, types, children);
}
...
{
	std::string path = "/home/bram/projects/catkin_projects/SQUIRREL/";
	std::string domain_file = "know_rule_domain.pddl";
	std::string problem_file = "know_rule_problem.pddl";
	std::string robot_location_predicate = "kenny_waypoint";
	
	std::map<std::string, std::string> object_to_location_mapping;
	std::map<std::string, std::vector<std::string> > near_waypoint_mappings;
	std::vector<std::string> toy1_near;
	toy1_near.push_back("near_toy1");
	near_waypoint_mappings["kenny_waypoint"] = toy1_near;
	
//	std::vector<std::string> toy2_near;
//	toy2_near.push_back("near_toy2");
//	near_waypoint_mappings["kenny_waypoint"] = toy2_near;
	
	std::vector<std::string> box1_near;
	box1_near.push_back("near_box1");
	near_waypoint_mappings["box1_location"] = box1_near;
	
	std::vector<std::string> box2_near;
	box2_near.push_back("near_box2");
	near_waypoint_mappings["box2_location"] = box2_near;
	
	std::map<std::string, std::string> object_to_type_mapping;
	object_to_type_mapping["toy1"] = "dinosaur";
//	object_to_type_mapping["toy2"] = "car";
	
	std::map<std::string, std::string> box_to_location_mapping;
	box_to_location_mapping["box1"] = "box1_location";
	box_to_location_mapping["box2"] = "box2_location";
	std::map<std::string, std::string> box_to_type_mapping;
	box_to_type_mapping["box1"] = "dinosaur";
	box_to_type_mapping["box2"] = "car";
...
//	children.push_back("child2");
	...
	std::vector<std::string> child1_near;
	child1_near.push_back("near_child1");
	near_waypoint_mappings["child1_location"] = child1_near;
	
//	std::vector<std::string> child2_near;
//	child2_near.push_back("near_child2");
//	near_waypoint_mappings["child2_location"] = child2_near;
	
	std::map<std::string, std::string> object_to_child_mapping;
	object_to_child_mapping["toy1"] = "child1";
//	object_to_child_mapping["toy2"] = "child2";
	
	std::map<std::string, std::string> child_location_mapping;
	child_location_mapping["child1"] = "child1_location";
//	child_location_mapping["child2"] = "child2_location";
	
	KCL_rosplan::RobotKnowsSortingGamePDDLGenerator generator;
	generator.createPDDL(path, domain_file, problem_file, robot_location_predicate, object_to_location_mapping, near_waypoint_mappings, object_to_type_mapping, box_to_location_mapping, box_to_type_mapping, children, object_to_child_mapping, child_location_mapping);
}
